...
def _compute_log_a_for_int_alpha(sampling_rate, sigma, alpha):
    ...
    for i in range(alpha + 1):
        log_coef_i = (
                ...
                + i * math.log(sampling_rate)
                + (alpha - i) * math.log(1 - sampling_rate)
        )
...
def _compute_log_a_for_frac_alpha(sampling_rate, sigma, alpha):
    ...
    z0 = sigma ** 2 * math.log(1 / sampling_rate - 1) + 0.5
    ...
    while True:  # do ... until loop
        ...
        log_t0 = log_coef + i * math.log(sampling_rate) + j * math.log(1 - sampling_rate)
        log_t1 = log_coef + j * math.log(sampling_rate) + i * math.log(1 - sampling_rate)
...
def _compute_log_a(sampling_rate, sigma, alpha):
    """Computes :math:`log(A_\alpha)` for any positive finite ``alpha``.
    Notes:
        Note that
        :math:`A_\alpha` is real valued function of ``alpha`` and ``q``,
        and that 0 < ``q`` < 1.
    ...
    if float(alpha).is_integer():
        return _compute_log_a_for_int_alpha(sampling_rate, sigma, int(alpha))
    else:
        return _compute_log_a_for_frac_alpha(sampling_rate, sigma, alpha)


def _compute_rdp(sampling_rate, sigma, alpha):
    r"""Computes RDP of the Sampled Gaussian Mechanism at order ``alpha``.
    Args:
        sampling_rate: Sampling rate of SGM.
        sigma: The standard deviation of the additive Gaussian noise.
        alpha: The order at which RDP is computed.
    Returns:
    ...
    return _compute_log_a(sampling_rate, sigma, alpha) / (alpha - 1)


def compute_rdp(sampling_rate, noise_multiplier, steps, orders):
    ...
    if isinstance(orders, float):
        rdp = _compute_rdp(sampling_rate, noise_multiplier, orders)
    else:
        rdp = np.array([_compute_rdp(sampling_rate, noise_multiplier, order) for order in orders])
...
def compute_epsilon(sigma, sampling_prob_1, global_iterations, delta, verbose=False):
    orders = np.arange(2, 50, 0.1)
    rdp_1 = np.atleast_1d(compute_rdp(sampling_prob_1, sigma, global_iterations, orders))
    eps, best_alpha = get_privacy_spent(orders, rdp_1, delta)
    return float(eps)


# taken and modified from pytorch/opacus
def get_noise_multiplier(target_epsilon,target_delta,sample_rate,steps,epsilon_tolerance):
    """
    ...
    while eps_high > target_epsilon:
        ...
        eps_high = compute_epsilon(sigma_high, sampling_prob_1=sample_rate, global_iterations=steps, delta=target_delta)
        if sigma_high > MAX_SIGMA:
            raise ValueError("The privacy budget is too low.")

    while target_epsilon - eps_high > epsilon_tolerance:
        sigma = (sigma_low + sigma_high) / 2
        eps = compute_epsilon(sigma, sampling_prob_1=sample_rate, global_iterations=steps, delta=target_delta)
